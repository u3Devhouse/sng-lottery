...
describe("Lottery", function ()
{
  ...
  describe("Before Initialized", () =>
  {
    ...
    it("Should have the appropriate pot distribution amounts", async () =>
    {
      const { lottery } = await loadFixture(setupStarted);
      const dist1Info = await lottery.roundDistribution(1)
      expect(dist1Info[ 0 ]).to.equal(parseEther("2500"));
      expect(dist1Info[ 1 ]).to.equal(parseEther("2500"));
      expect(dist1Info[ 2 ]).to.equal(parseEther("2500"));
  ...
  describe("Claim Winnings", () =>
  {
    ...
    {
      const { lottery, user3, user5 } = await loadFixture(setupRoundEnded);
      ...
      await expect(lottery.connect(user3).claimTickets(1, [ 0, 0 ], [ 4, 4 ])).to.be.revertedWithCustomError(lottery, "BlazeLot__DuplicateTicketIdClaim").withArgs(1, 0); // args( round id,ticket index, match)
